...
function ElementMeasurement() {
  ...
  return (
    <div>
      ...
      >
        <p>Cette boîte est redimensionnable !</p>
        <p>Tire le coin en bas à droite pour la redimensionner.</p>
        <p>Contenu qui dépasse...</p>
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        <button onClick={scrollToTop}>⬆️ Scroll Top</button>
        <button onClick={scrollToBottom}>⬇️ Scroll Bottom</button>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        <button onClick={saveCanvas}>💾 Sauvegarder</button>
        <button onClick={() => drawPreset('circle')}>⭕ Cercle</button>
        <button onClick={() => drawPreset('square')}>⬜ Carré</button>
        <button onClick={() => drawPreset('line')}>📏 Ligne</button>
      </div>
    </div>
  )
}
...
function MutableValues() {
  ...
  // Références pour stocker des valeurs mutables
  ...
  const handleInputChange = (e) => {
    // Stocker la valeur sans déclencher de re-render
    inputValueRef.current = e.target.value
    console.log('Valeur stockée (sans re-render):', inputValueRef.current)
  }

  const showStoredValue = () => {
    setMessage(`Valeur stockée: "${inputValueRef.current}"`)
  }
  ...
  return (
    <div>
      ...
      <div>
        ...
        <p>Count précédent: {previousCountRef.current}</p>
        <p>Timer status: {timerRef.current ? '🟢 Actif' : '🔴 Arrêté'}</p>
      </div>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        />
        <button onClick={showStoredValue}>Afficher valeur stockée</button>
        {message && <p>{message}</p>}
      </div>
    </div>
  )
...
function CacheExample() {
  ...
  const searchData = async (searchQuery) => {
    ...
    if (cacheRef.current.has(searchQuery)) {
      console.log('🎯 Résultat du cache pour:', searchQuery)
    ...
    try {
      console.log('🌐 Requête API pour:', searchQuery)
      
      // Simulation d'une API call
      await new Promise(resolve => setTimeout(resolve, 1000))
      
      const mockResults = [
        `Résultat 1 pour "${searchQuery}"`,
        `Résultat 2 pour "${searchQuery}"`,
        `Résultat 3 pour "${searchQuery}"`
      ]
  ...
  const showCacheInfo = () => {
    console.log('📊 Contenu du cache:', Object.fromEntries(cacheRef.current))
    alert(`Cache contient ${cacheRef.current.size} entrées`)
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        <button onClick={showCacheInfo}>📊 Info Cache</button>
        <span style={{ marginLeft: '10px' }}>
          Cache: {cacheRef.current.size} entrées
        </span>
      </div>
      ...
      {sections.map((sectionId, index) => (
        ...
        >
          ...
          <p>
            ...
            {visibleSections.has(sectionId) ? ' ✅ Visible' : ' ❌ Non visible'}
          </p>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
        </section>
      ))}
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        <button onClick={selectAllText}>Sélectionner tout</button>
      </div>
    </div>
  )
})
...
const AdvancedInput = forwardRef((props, ref) => {
  ...
  // Exposer des méthodes personnalisées
...
function ImperativeHandleExample() {
  ...
  const handleSetValue = () => {
    advancedInputRef.current.setValue('Valeur définie par le parent')
  }

  const handleGetSelection = () => {
    const selection = advancedInputRef.current.getSelection()
    alert(`Sélection: ${selection.start}-${selection.end}`)
  }
  ...
  return (
    <div>
      ...
      <AdvancedInput
        ...
        placeholder="Input avec méthodes personnalisées"
      />
...
1. **useRef** = références DOM + valeurs mutables
2. **Pas de re-render** = changer `.current` ne déclenche pas de re-render
3. **DOM access** = accéder directement aux éléments HTML
4. **Valeurs persistantes** = survivent aux re-renders
5. **Cleanup** = penser à nettoyer les timers/listeners stockés en ref
6. **forwardRef** = passer des refs aux composants enfants
7. **useImperativeHandle** = exposer des méthodes personnalisées

---

## Erreurs Communes

### 1. Accès à ref.current avant le mount

```jsx
// ❌ MAUVAIS - ref peut être null
function BadRefUsage() {
  const inputRef = useRef(null)
  
  // ❌ Peut crash si appelé avant le mount
  const focusInput = () => {
    inputRef.current.focus() // ❌ Peut être null !
  }
...
```jsx
// ❌ MAUVAIS - Utiliser ref pour l'état UI
function BadStateUsage() {
  const countRef = useRef(0)
  
  const increment = () => {
    countRef.current++ // ❌ Pas de re-render !
  }
  
  return (
    <div>
      <p>Count: {countRef.current}</p> {/* ❌ Ne se met pas à jour */}
      <button onClick={increment}>+1</button>
    </div>
  )
}

// ✅ BON - Utiliser state pour l'état UI
...
**Prochaine étape :** L'optimisation avec `useMemo` et `useCallback` dans `15.0.4.3-useMemo-useCallback.md` ! ⚡
