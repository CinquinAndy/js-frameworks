...
Maintenant qu'on comprend le rendu et le Virtual DOM, on va plonger dans **les hooks** - la révolution qui a changé React pour toujours !

Les **hooks** ont été introduits dans React 16.8 et permettent d'utiliser les fonctionnalités de React (comme gérer l'état du composant ou effectuer des effets de bord) **sans écrire de classes**.
...
```jsx
// ❌ AVANT - Class Component (2015-2019)
...
// ✅ MAINTENANT - Functional Component avec Hooks (2019+)
...
2. **Plus lisible** - Logique groupée par fonctionnalité
3. **Réutilisable** - Custom hooks pour partager la logique
4. **Performance** - Optimisations automatiques
5. **TypeScript friendly** - Meilleure inférence de types

### Règles des Hooks ⚠️

```jsx
function RèglesHooks() {
  // ✅ CORRECT - Appelés au top level
  const [count, setCount] = useState(0)
  const [name, setName] = useState('')

  // ❌ INCORRECT - Dans une condition
  if (count > 5) {
    // const [error, setError] = useState('') // ❌ NE JAMAIS FAIRE ÇA
  }

  // ❌ INCORRECT - Dans une boucle
  for (let i = 0; i < 3; i++) {
    // const [items, setItems] = useState([]) // ❌ NE JAMAIS FAIRE ÇA
  }

  // ❌ INCORRECT - Dans une fonction imbriquée
  const handleClick = () => {
    // const [temp, setTemp] = useState(0) // ❌ NE JAMAIS FAIRE ÇA
  }

  return <div>Respectez les règles !</div>
}
```
...
function MiseAJourAvancee() {
  ...
  // ✅ BON - Utilise une fonction de mise à jour
  const incrementGood = () => {
    setCount(prev => prev + 1)
    setCount(prev => prev + 1) // Fonctionne correctement !
  }

  // ✅ BON - Pour des calculs complexes
  const complexUpdate = () => {
    setCount(prevCount => {
...
function GestionComplexe() {
  ...
  // ✅ Mise à jour d'objet profondément imbriqué
  const toggleTheme = () => {
    setUser(prevUser => ({
      ...prevUser,
      preferences: {
...
function OptimisationInit() {
  ...
  return (
    <div>
      <p>State optimisé: {optimizedState}</p>
...
function EffectPatterns() {
  ...
  useEffect(() => {
    ...
    // Cleanup crucial pour éviter les fuites mémoire
    return () => {
      clearInterval(timer)
...
function EffectDependencies({ userId, filter }) {
  ...
  // ❌ MAUVAIS - Objet recréé à chaque rendu
  useEffect(() => {
...
function UseRefBasics() {
  ...
  // Incrément silencieux (pas de re-rendu)
...
function AdvancedRefUsage() {
  ...
  return (
    <div>
      ...
      >
        {isVisible ? 'Je suis visible ! 👀' : 'Je ne suis pas visible 🙈'}
      </div>
    </div>
  )
}
...
function UseMemoExample({ items, filter }) {
  ...
  // ❌ MAUVAIS - Calcul à chaque rendu
  const expensiveValue = items
    .filter(item => item.category === filter)
    .reduce((sum, item) => sum + item.price, 0)

  // ✅ BON - Calcul seulement si items ou filter changent
  ...
  const chartData = useMemo(() => {
    console.log('📊 Génération des données chart')
    return {
  ...
  return (
    <div>
      ...
      <p>Total (non mémorisé): {expensiveValue}</p>
      <p>Total (mémorisé): {memoizedValue}</p>
...
const ExpensiveChart = memo(function ExpensiveChart({ data }) {
  console.log('📈 ExpensiveChart re-rendu')

  return (
    <div style={{ border: '1px solid #ccc', padding: '10px' }}>
      <h4>Chart (mémorisé)</h4>
      <pre>{JSON.stringify(data, null, 2)}</pre>
    </div>
  )
})
...
const TodoForm = memo(function TodoForm({ onAddTodo, onAddTodoWithPrefix }) {
  ...
  return (
    <form>
      ...
      <button type="button" onClick={handleSubmitWithPrefix}>Ajouter avec préfixe</button>
    </form>
  )
})
...
```jsx
// ❌ useState devient complexe avec plusieurs états liés
function ComplexStateWithUseState() {
  ...
  // ... beaucoup de logique dispersée
}

// ✅ useReducer centralise la logique
...
### Exemple avancé : Todo App avec useReducer
...
## useContext - Partage de Données

### Problème résolu par useContext
    ...
    <div style={{ border: '1px solid #ccc', padding: '20px', borderRadius: '5px' }}>
      ...
      {user.preferences.notifications ? (
        <div>
          ...
          <ul>
            <li>Nouveau message reçu</li>
            <li>Mise à jour disponible</li>
...
function AuthenticatedApp() {
  ...
  const handleAction = (action) => {
    addNotification(`Action "${action}" exécutée`, 'info')
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
        <button onClick={() => handleAction('Créer un document')}>
          Créer un document
        </button>
        <button onClick={() => handleAction('Voir les rapports')}>
          Voir les rapports
        </button>
        <button onClick={() => handleAction('Gérer les utilisateurs')}>
...
function UseLayoutEffectExample() {
  ...
  // ⚠️ useEffect - S'exécute APRÈS le paint
  useEffect(() => {
    console.log('📐 useEffect - mesure après paint')
  })

  // ✅ useLayoutEffect - S'exécute AVANT le paint
  useLayoutEffect(() => {
    console.log('📏 useLayoutEffect - mesure avant paint')
...
function PerformanceHooks() {
  ...
  const searchResults = useMemo(() => {
    ...
    console.log('🔍 Recherche pour:', deferredQuery)

    // Simulation d'un calcul coûteux
    return Array.from({ length: 1000 }, (_, i) => ({
      id: i,
      title: `Résultat ${i} pour "${deferredQuery}"`
    })).filter(item =>
  ...
  const handleInputChange = (value) => {
    // Mise à jour urgente de l'input
    setQuery(value)

    // Mise à jour différée pour la recherche
    startTransition(() => {
      // Cette mise à jour sera moins prioritaire
      console.log('🔄 Transition démarrée pour:', value)
    })
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '20px' }}>
        <p>Résultats pour "{deferredQuery}": {searchResults.length} trouvés</p>
        ...
        <div style={{ maxHeight: '300px', overflow: 'auto' }}>
          ...
          {searchResults.length > 50 && (
            <p>... et {searchResults.length - 50} autres résultats</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        <div>
          <h4>Utilisateurs</h4>
          <button onClick={refetchUsers}>🔄 Rafraîchir</button>
          <ul>
        ...
        <div>
          <h4>Posts</h4>
          <button onClick={refetchPosts}>🔄 Rafraîchir</button>
          <ul>
            {posts?.map(post => (
              <li key={post.id}>{post.title}</li>
            ))}
...
function useLocalStorage(key, initialValue) {
  ...
  const setValue = useCallback((value) => {
    try {
      // Permettre à value d'être une fonction pour être compatible avec useState
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
        ...
        <div>
          ...
          <button onClick={removePreferences}>Reset préférences</button>
        </div>
...
```jsx
// Hook composé utilisant plusieurs hooks
...
1. **Hooks = fonctions spéciales** qui commencent par "use"
2. **Règles des hooks** = toujours au top level, même ordre
3. **useState** = état local, mise à jour asynchrone
4. **useEffect** = effets de bord, cleanup crucial
5. **useRef** = références DOM et valeurs mutables
6. **useMemo/useCallback** = optimisations performance
7. **useReducer** = gestion d'état complexe
8. **useContext** = partage de données sans prop drilling
9. **Custom hooks** = réutiliser la logique entre composants
...
- 📚 [Référence complète des Hooks](https://react.dev/reference/react)
- 🎯 [useEffect en détail](https://react.dev/reference/react/useEffect)
- 💾 [useState patterns](https://react.dev/reference/react/useState)
- 🔄 [useReducer guide](https://react.dev/reference/react/useReducer)
- 🌐 [useContext patterns](https://react.dev/reference/react/useContext)
- ⚡ [useMemo optimisations](https://react.dev/reference/react/useMemo)
- 🎛️ [useCallback guide](https://react.dev/reference/react/useCallback)

**Prochaine étape :** Le routing avec React Router dans `15.0.5-Router.md` !