...
Salut ! Maintenant qu'on maîtrise les composants, hooks et routing, on va s'attaquer à un sujet **crucial** : la **gestion d'état** !

Quand ton app grandit, gérer l'état avec juste `useState` devient vite le chaos. On va voir comment **structurer** et **partager** l'état efficacement.
...
Avec `useState` seul, tu vas vite avoir ces problèmes :
...
1. **Context API** → pour partager l'état sans prop drilling
2. **Zustand** → state manager simple et moderne
3. **Redux Toolkit** → pour les grosses apps complexes
4. **Jotai** → approche atomique
5. **TanStack Query** → pour les données serveur
...
| Solution | Simplicité | Performance | Fonctionnalités | Écosystème |
|----------|------------|-------------|-----------------|------------|
| **useState** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |
| **Context API** | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **Zustand** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
...
#### 🏢 **Apps moyennes (10-50 composants)**
```jsx
...
#### 🏭 **Apps moyennes/grandes (50+ composants)**
```jsx
// Zustand pour l'état client + TanStack Query pour les données serveur
...
#### 🏗️ **Grosses apps enterprise (100+ composants)**
```jsx
...
src/
├── stores/
│   ├── useAuthStore.js      # Auth avec Zustand
│   ├── useCartStore.js      # Panier avec Zustand
│   └── useUIStore.js        # UI state avec Zustand
├── api/
│   ├── products.js          # TanStack Query pour produits
│   ├── orders.js            # TanStack Query pour commandes
│   └── auth.js              # TanStack Query pour auth
├── components/
└── pages/
```
...
### 1. Séparer état local vs global

```jsx
// ❌ Tout dans le store global
const useAppStore = create((set) => ({
  modalOpen: false,
  searchQuery: '',
...
// ✅ État local pour l'UI, global pour les données partagées
...
function CartSummary() {
  ...
  return <div>Total: {total}€ ({items.length} articles)</div>
}
```
...
function CartSummary() {
  ...
  return (
    <div>
      {cartData.itemCount} articles - {cartData.total}€
    </div>
  )
}
```
...
```jsx
// Étape 1: Garder l'ancien Context, ajouter Zustand pour nouvelles features
function App() {
  return (
...
// Étape 2: Migrer petit à petit
function Header() {
  // Ancien code (à supprimer plus tard)
...
// Étape 3: Supprimer l'ancien Context quand tout est migré
```
...
1. **useState** → pour l'état local d'un composant
2. **Context API** → pour partager l'état sans prop drilling
3. **Zustand** → store simple et performant
4. **TanStack Query** → pour les données serveur
5. **Redux Toolkit** → pour les apps complexes

### Architecture recommandée

```jsx
// 🎯 Stack moderne optimal
function App() {
  return (
    <QueryClientProvider client={queryClient}>  {/* Données serveur */}
      <Router>
        <Layout />  {/* Zustand pour l'état client */}
      </Router>
    </QueryClientProvider>
  )
}
...
1. **Commencez simple** : useState → Context → Zustand
2. **Séparez les préoccupations** : local vs global, client vs serveur
3. **Optimisez les sélecteurs** : ne prenez que ce dont vous avez besoin
4. **Typez votre état** : TypeScript pour éviter les bugs
5. **Testez vos stores** : logique métier séparée des composants
...
**Prochaine étape :** Maintenant que tu maîtrises la gestion d'état, on va voir comment optimiser les performances et gérer les rendus complexes dans le prochain chapitre ! 🚀
