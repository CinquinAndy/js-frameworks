...
## Hooks de Données et API

### useFetch - Appels API Simplifiés
...
function useApi(baseUrl) {
  ...
  const get = useCallback(async (endpoint, useCache = true) => {
    ...
    if (useCache && cache.has(fullUrl)) {
      console.log('📦 Données depuis le cache:', fullUrl)
      return cache.get(fullUrl)
    }

    console.log('🌐 Fetch API:', fullUrl)
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <p>Taille du cache: {api.cacheSize} entrées</p>
        <button onClick={loadPosts} disabled={postsLoading}>
          {postsLoading ? 'Chargement...' : 'Charger Posts'}
        </button>
        <button onClick={api.clearCache}>🗑️ Vider cache</button>
  ...
  return (
    <div>
      ...
      <div style={{ 
        ...
        <p>Device: {breakpoint.isMobile ? '📱 Mobile' : '🖥️ Desktop'}</p>
        
        {breakpoint.isXs && <p>📱 Très petit écran</p>}
        {breakpoint.isSm && <p>📱 Petit écran</p>}
        {breakpoint.isMd && <p>💻 Écran moyen</p>}
        {breakpoint.isLg && <p>🖥️ Grand écran</p>}
        {breakpoint.isXl && <p>🖥️ Très grand écran</p>}
      </div>
    </div>
  )
}
...
function DebounceDemo() {
  ...
  useEffect(() => {
    if (debouncedSearchTerm) {
      ...
      setTimeout(() => {
        const mockResults = [
          `Résultat 1 pour "${debouncedSearchTerm}"`,
          `Résultat 2 pour "${debouncedSearchTerm}"`,
          `Résultat 3 pour "${debouncedSearchTerm}"`
        ]
    ...
    <div>
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <textarea
          ...
          placeholder="Écris quelque chose... sauvegarde automatique après 1s"
          rows={4}
          style={{ width: '100%' }}
        />
        <p>{saveStatus}</p>
...
function TimerDemo() {
  ...
  const countdown = useTimer(60) // Démarre à 60 secondes
  
  // Pour le countdown, on décrémente
  useInterval(
    () => {
      if (countdown.time > 0) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '30px' }}>
        <h4>Notifications Périodiques (toutes les 10s)</h4>
        <ul>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h2>Panier</h2>
              <button onClick={() => cart.setIsOpen(false)}>❌</button>
            </div>
...
2. **Réutilisabilité** = extraire la logique commune
3. **Composition** = combiner plusieurs hooks simples
4. **Règles des hooks** = respecter les mêmes règles que les hooks natifs
...
```jsx
// ✅ BON - Retourner un objet avec noms explicites
...
// ✅ BON - Retourner un array pour les cas simples (comme useState)
...
// ❌ ÉVITER - Mélanger objets et arrays
...
**Félicitations ! Tu maîtrises maintenant tous les hooks React ! 🎉**
