...
### Récupérer les paramètres avec useParams
...
function LoginForm() {
  ...
  const handleSubmit = (e) => {
    ...
    // Ou redirection en remplaçant l'historique
...
function Analytics() {
  ...
  useEffect(() => {
    ...
    console.log('Page visitée :', location.pathname)
    console.log('Paramètres :', location.search)
  ...
  const [posts, setPosts] = useState([
    {
      ...
      excerpt: 'Voici le début de mon blog...',
    ...
    <div className="login-page">
      <form onSubmit={handleSubmit} className="login-form">
        ...
        <p className="login-hint">
          💡 Utilisez admin@example.com / password
        </p>
      </form>
    </div>
  )
...
### Configuration avec routes protégées
...
function Navigation() {
  const preloadRoute = async (routeModule) => {
    // Précharger le composant au survol
    await routeModule()
  }
...
#### Reach Router (fusionné avec React Router)

```jsx
// ❌ Plus maintenu (fusionné dans React Router v6)
...
2. **Routes + Route** = définir les chemins et composants
3. **Link/NavLink** = navigation sans rechargement
4. **useParams** = récupérer les paramètres d'URL
5. **useNavigate** = navigation programmatique
6. **Outlet** = afficher les routes imbriquées

### Les bonnes pratiques

1. **Structure claire** : séparez vos pages dans un dossier dédié
2. **Routes protégées** : vérifiez l'authentification
3. **404 personnalisée** : route `path="*"` en dernier
...
- ❌ Oublier la route 404 (`path="*"`)
- ❌ Paramètres d'URL non validés
- ❌ Navigation sans gestion d'erreur
- ❌ Routes imbriquées mal structurées
...
**Prochaine étape :** Maintenant que tu maîtrises React Router, on va s'attaquer à la gestion d'état avancée avec Context API et les state managers comme Zustand dans le prochain chapitre ! 🚀
