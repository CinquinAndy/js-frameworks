...
**useContext** = Fini le **prop drilling** ! 🌐
    ...
    <div style={{
      ...
      <header style={{ 
        ...
        <div>
          ...
          <p>Thème: {settings.theme} | Langue: {settings.language}</p>
        </div>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
        <button onClick={() => handleAction('Créer document')}>
          📄 Créer document
        </button>
        <button onClick={() => handleAction('Voir rapports')}>
          📊 Voir rapports
        </button>
        <button onClick={() => handleAction('Gérer utilisateurs')}>
          👥 Gérer utilisateurs
        </button>
      </div>
...
function TodoStats() {
  ...
  return (
    <div style={{ marginTop: '20px', padding: '10px', backgroundColor: '#f8f9fa' }}>
      ...
      <p>Terminées: {computed.completedCount}</p>
    </div>
  )
}
```
...
```jsx
// ❌ PROBLÈME - Context value recréé à chaque render
...
// ✅ ENCORE MIEUX - Séparer les contexts
const UserContext = createContext()
const SettingsContext = createContext()
...
// Les composants ne re-render que si leur context spécifique change !
```

### Context Selector Pattern

```jsx
// Pattern avancé pour optimiser les performances
...
// Ces composants ne re-render que si leur partie spécifique change
function Header() {
...
1. **useContext** = partage de données sans prop drilling
2. **createContext** = créer le context
3. **Provider** = fournir les données aux enfants
4. **Custom hook** = encapsuler useContext + validation
5. **Multiples contexts** = diviser pour mieux régner
6. **Performance** = mémoriser les valeurs avec useMemo
...
└── index.js          // Export tous les contexts
...
providers/
└── AppProviders.js    // Combine tous les providers
```

### Patterns à Suivre

```jsx
// ✅ Toujours valider le context
...
// Auth, Settings, Notifications = contexts séparés

// ✅ Mémoriser les valeurs complexes
const value = useMemo(() => ({
  state,
  actions
}), [state, actions])

// ✅ Actions creators pour simplifier l'usage
const actions = useMemo(() => ({
...
```jsx
// ❌ Erreur - Utiliser le context sans Provider
function App() {
  return <ComponentUsingContext /> // ❌ Crash !
}

// ✅ Correct - Wrapper avec le Provider
function App() {
  return (
    <MyProvider>
...
```jsx
// ❌ Mauvais - Re-render tous les enfants
...
**Prochaine étape :** Les hooks avancés dans `15.0.4.6-hooks-avances.md` ! 🚀
