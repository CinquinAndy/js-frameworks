...
1. **Rendu Déclaratif** = tu dis QUOI, React s'occupe du COMMENT
2. **3 phases** = Trigger → Render → Commit
3. **Virtual DOM** = optimisation automatique des mises à jour
4. **Re-rendus** = seulement si état/props changent
5. **useEffect** = cycles de vie des composants fonctionnels
6. **Refs** = "porte de sortie" pour sortir du flux React
7. **Optimisations** = memo, useMemo, useCallback pour les performances
8. **Pureté** = les composants doivent être des fonctions pures
...
**Prochaine étape :** Les hooks avancés (useEffect, useContext, useReducer...) dans `15.0.4-Hooks.md` !